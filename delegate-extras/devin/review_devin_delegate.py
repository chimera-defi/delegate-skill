...
def repo_root_from_script() -> Path:
    # Resolve the repo under review from the invocation cwd (git), not from this
    # script's own location: once this file lives in the shared
    # delegate-extras/devin/ install path, parents[1] no longer points at the
    # delegate repo. Fall back to the old relative path when git is unavailable.
    proc = subprocess.run(
