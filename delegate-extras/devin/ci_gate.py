...
try:
    sys.path.insert(0, str(_Path(__file__).resolve().parents[1] / "shared"))
    from aggregate_telemetry import load_events, summarize  # type: ignore
    _HAVE_AGG = True
except Exception:
    _HAVE_AGG = False
...
def gate(
    ...
    tele = summarize(load_events(days=days, delegate="devin-delegate")) if _HAVE_AGG else {"delegate_calls": 0}
